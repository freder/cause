...
function list_tasks() {
	...
	tasks.forEach(function(task, index) {
		console.log( sf('({0}) {1}', index, helper.format_msg(task.name, task.interval)) );
	});
	console.log(HR);
}
...
function show_version() {
	var pkg = require(path.join(global.paths.root, 'package.json'));
...
function log_price_delta(price, prev_price, task) {
	...
	winston.info( sf('{0} | {1}', helper.format_msg(task.name, delta), message) );
}
