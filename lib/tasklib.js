...
function normalize_step_data(step, defaults) {
	return normalize(step.data || {}, defaults || {});
}
function normalize_step_options(step, defaults) {
	return normalize(step.options || {}, defaults || {});
}


function save_task(task) {
	if (!task._file) {
		winston.error("can't save task — don't know where to.");
		return;
	}
...
function create_step(block, task, step) {
	...
	step.options = normalize_step_options(step, block.defaults);
	step.data = normalize_step_data(step, block.data_defaults);
