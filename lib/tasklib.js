...
function loadTaskFromFile(taskFileAbsPath, cb) {
	fs.readFile(taskFileAbsPath, (err, data) => {
		...
		taskData._file = taskFileAbsPath;
		cb(null, taskData);
	});
};
...
function createExecuteFunction(block, task, step) {
	return function(input, prevStep, cb) {
		...
		const done = function(err, output, decision) {
			...
			// invoke child steps
			invokeChildren(step, task, output, decision);
...
function saveTask(task) {
	if (!task._file) {
		debug(`can't save task "${task.name}" — no file specified.`);
		return;
	}
	debug(`saving task "${task.name}" to "${path.relative(cwd, task._file)}"`);
	fs.writeFileSync(
		task._file,
		JSON.stringify(makeSavable(task), null, '\t')
	);
	return task;
};
...
function invokeChildren(step, task, output, flowDecision) {
	const input = output; // prev step's output is next step's input
		...
		.forEach(function(key) {
			...
			try {
				step.flow[key] // children ids
					.forEach(function(id) {
						// TODO: use getItemById from other project
						const childStep = utils.misc.get_first_by(task.steps, 'id', id);
						childStep._execute(input, step);
					});
			} catch (e) {
