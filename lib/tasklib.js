...
function createExecuteFunction(block, task, step) {
	return (input, prevStep, doneCb=defaultDoneCb) => {
		...
		const context = createContext(step, prevStep, task);
		const _config = prepareStepConfig(config, step, task, block.name);
		block.fn(step, context, _config, input, wrappedDoneCb);
	};
};
...
function createContext(step, prevStep, task) {
	return {
		prevStep, task,
		prepareDecision,
		utils,
