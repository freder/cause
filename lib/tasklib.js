...
function create_step_function(block, task, step) {
	...
	task = _.defaults(task, {
		// keep track of which steps are currently executing
		_currently_executing_steps: {}
	});
...
function run_task(task) {
	var debug = require('debug')('cause:');
	debug( sf('{0} {1}', chalk.bgBlue(task.name), chalk.bgWhite.blue('START')) );
	task._done = task._done || function() {
		debug( sf('{0} {1}', chalk.bgBlue(task.name), chalk.bgWhite.blue('END')) );
	};
